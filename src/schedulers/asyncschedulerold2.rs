            ...
            loop {
                match (*recv.borrow_mut()).recv().await {
                    Some(task) => {
                        ...
                        // let r1_clone_ = Rc::clone(&r1);
                        // let r1_clone__ = r1.borrow_mut
                        // let r1_clone = Rc::clone(&r1);
                        // let rc_s_clone_ = *r1_clone;
                        // r1_val = Rc::try_unwrap(r1_clone).unwrap_err()

                        let counter = Rc::strong_count(&r1);
                            ...
                            Ok(r1_val) => {
                                ...
                                tokio::task::spawn_local(async move {
                                    ...
                                    match r2_result {
                                        ...
                                        Err(msg) => panic!("Not good2"),
                                    }
                                    // task.run(r2_clone)
                                });
                            }
                            Err(msg) => panic!("Not good1"),
                        }
